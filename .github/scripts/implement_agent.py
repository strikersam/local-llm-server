...
def tool_read_file(path: str) -> str:
    try: return Path(path).read_text(errors="replace")[:12000]
    except Exception as e: return f"[error: {e}]"
...
def main():
    ...
    while turns < MAX_TURNS:
        ...
        for tc in msg.tool_calls:
            ...
            if tc.function.name == "bash" and "pytest" in args.get("cmd", ""): last_pytest_passed = "[exit 0]" in out
            if tc.function.name == "bash" and "IMPLEMENTATION_COMPLETE" in out:
                if last_pytest_passed: success = True
                else: out = "[ERROR] Pytest failed. Fix tests before completing."
