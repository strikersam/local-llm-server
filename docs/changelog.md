...
### Fixed
- **Dashboard runtimes page could not start local agent wrappers, and dashboard-created tasks were not being picked up** — `backend/server.py` now starts the shared `RuntimeManager` and `TaskDispatcher` during app lifespan, matching `proxy.py`, so runtime health is polled on `:8001` and queued tasks are executed from the dashboard backend too. `Dockerfile.backend` now copies `docker/agent_runtime.py`, which fixes the `agent_runtime.py wrapper script not found` fallback path. The lightweight runtime wrapper now also exposes Hermes-compatible `/tasks` and OpenCode-compatible `/run` execution endpoints, and it falls back to an installed Ollama model when a requested default such as `qwen3-coder:30b` is not present locally.
- **Live v3 auth tests failed with `401 Invalid credentials`** — `backend/server.py` now treats `V3_ADMIN_EMAIL` / `V3_ADMIN_PASSWORD` as the effective dashboard login credentials when present, including admin seeding and password resync. This aligns the live integration tests, the v3 auth tests, and the actual dashboard login flow.
...
### Added
- **Task workflow regression coverage (`tests/test_tasks_workflow.py`)** — Added lifecycle, comment/review, execution-coordinator, scheduler-origin, and end-to-end task API coverage for the Multica-style workflow.
...
### Fixed
- **GitHub Pages login/bootstrap flow (`frontend/src/App.js`, `frontend/src/api.js`, `frontend/src/pages/LoginPage.js`, `frontend/src/pages/SettingsPage.js`)** — Added a public pre-login setup route so static deployments can configure a backend before authentication, made social-login controls respect the active backend URL, fixed token refresh to use the currently configured backend instead of the build-time URL, corrected post-auth redirects to stay under the deployed `PUBLIC_URL`, and ensured GitHub OAuth popup origin checks use the live backend configuration.
- **Router alias fallback regression (`router/model_router.py`)** — Explicit `MODEL_MAP` aliases such as `claude-opus-4-6` no longer silently downgrade to unrelated installed models when the preferred local model is unavailable; only short local aliases are rewritten through availability fallback, restoring deterministic alias routing and the expected test behavior.
...
- Fix two incorrect assertions in `tests/test_secrets.py` that expected standard `USER` role to read workspace-scoped secrets; per the module spec (`WORKSPACE` is "power users + admins"), non-owner USERs must be denied access.
