...
- `proxy.py` — Added `GET /api/audit-log` endpoint backed by `rbac.get_audit_log()`. The frontend's `getAuditLog()` API call was hitting 404 because no HTTP route existed for the in-memory RBAC audit log. Supports filtering by `user_id`, `resource`, and `outcome`; requires authentication.
