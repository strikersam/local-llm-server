...
### Fixed
- `agent/workspace.py` — logger namespace corrected from `qwen-agent` to `qwen-proxy`. Duplicate `create()` now raises `WorkspaceError` instead of silently clobbering an existing workspace. TTL clock for `cleanup_after` now anchored to the moment a workspace enters a terminal state (via `transition()`) rather than creation time, so long-running jobs get their full TTL after completion. Blocking filesystem I/O (`cleanup_expired`, `clean_tmp`, mkdir/write in `create`, write in `transition`/`heartbeat`) moved to `asyncio.to_thread`.
