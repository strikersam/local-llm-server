...
### Fixed
- `frontend/src/App.js`, `frontend/src/pages/LoginPage.js`, `frontend/src/__tests__/appRouting.test.js`, and `frontend/src/__tests__/loginPage.test.js` — GitHub Pages deep links such as `/login` and `/companyhelm` now land on the real login experience again. The login screen no longer crashes during render from missing icon imports, and the temporary `/companyhelm` route now redirects back into the standard auth flow instead of hijacking the app.
