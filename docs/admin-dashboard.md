...
This is served by the same FastAPI process as the proxy (no separate hosting required).
...
> **Note:** A legacy static frontend exists in `remote-admin/` for remote access, but `/admin/app` is the recommended path.
