...
Add a browser-based chat + coding UI (Claw Code / Claude Code–style) to `local-llm-server`, with a protected admin area for configuration and management, and a deployment path that does not rely on any external static hosting provider.
...
- `docs/admin-dashboard.md` — point to the built-in web UI admin route.
